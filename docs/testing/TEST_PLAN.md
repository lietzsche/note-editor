...
4. 모바일 폭에서 편집 툴바가 제목/상태/액션을 노출하면서 본문 입력 영역을 과도하게 줄이지 않는지 확인

기대 결과:
- 입력/삭제 즉시 글자 수가 갱신됨
- 노트 전환 시 현재 노트 본문 기준 글자 수가 표시됨
- 에디터 본문과 표시 글자 수가 일치함
- 저장 상태와 글자 수가 편집기 상단에서 텍스트로 확인 가능함
...
5. 모바일/데스크톱에서 복사, 맞춤법, 공유 버튼이 보조 액션 영역에 유지되는지 확인

기대 결과:
- 한 번의 액션으로 전체 본문이 복사됨
- 성공/실패 피드백이 명확히 노출됨
- 빈 본문도 복사 액션이 허용되고 성공 피드백이 표시됨
- 보조 액션이 저장 상태와 본문 편집 흐름보다 강하게 보이지 않음
...
8. Confirm body searches with multiple matches show the current match position as `1 / N` and allow moving to previous/next match previews.
9. Confirm the user can hide the editor search context and reopen it while the same search is still active.
