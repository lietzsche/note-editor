...
이 기능은 일반 사용자용 `비밀번호 찾기`를 대체하는 운영자용 계정 복구 기능이다.
이번 범위는 username allowlist 기반 admin API와 감사 로그, 세션 무효화, 그리고 노트 화면 내부에서 여는 관리자 패널 UI를 포함한다.
...
- 역할(Role) 기반 관리자 시스템
- 별도 `/admin` 전용 라우트
- 다음 로그인 시 비밀번호 변경 강제 플로우

## 2. 사용자 시나리오

1. 운영자는 일반 로그인으로 서비스에 접속한다.
2. 운영자 username이 `ADMIN_USERNAMES` allowlist에 포함되어 있으면 노트 화면에서 운영 패널 버튼을 볼 수 있다.
3. 운영자는 운영 패널을 열어 사용자 목록을 보거나 username으로 검색한다.
4. 운영자는 대상 사용자의 비밀번호를 임시 비밀번호로 초기화한다.
5. 시스템은 임시 비밀번호를 반환하고, 대상 사용자의 기존 세션과 로그인 실패 기록을 정리한다.
6. 운영자는 별도 안전한 채널로 임시 비밀번호를 전달한다.
7. 운영자는 같은 패널에서 최근 초기화 감사 이력을 확인한다.
...
2. `관리자 패널`
- 운영 패널은 노트 화면 내부에서 슬라이드오버 패널로 연다.
- 일반 사용자에게는 운영 패널 진입 버튼을 노출하지 않는다.
- 패널 안에서 사용자 검색, 초기화 결과, 감사 로그를 한 번에 볼 수 있어야 한다.
- 현재 로그인한 운영자 자신의 계정은 UI에서 초기화 버튼을 비활성화한다.
...
5. `세션/로그인 시도 정리`
- 비밀번호 초기화 직후 대상 사용자의 기존 세션은 모두 무효화한다.
- 비밀번호 초기화 직후 대상 사용자의 `login_attempts` 기록을 정리해서 임시 비밀번호 로그인 시도를 막지 않도록 한다.
...
4. admin API 응답과 관리자 패널은 운영자에게만 노출되어야 하며 일반 사용자 UI와 연결되지 않는다.

## 8. 오픈 이슈

1. 운영자 전용 별도 라우트나 역할 모델로 확장할지 여부
2. 다음 로그인 시 강제 비밀번호 변경 플로우를 추가할지 여부
3. 운영자 자기 계정 초기화를 API 수준에서도 막을지 여부
