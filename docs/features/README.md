# Features Index

기능 범위 요구사항과 구현/테스트 기준을 관리한다.

## 문서 목록

1. `FEATURE-001-auth.md`
- 인증(회원가입/로그인/로그아웃/세션)

2. `FEATURE-002-notes-core.md`
- 노트 핵심 기능(CRUD/정렬)

3. `FEATURE-003-groups.md`
- 그룹(Group) 기반 관리

4. `FEATURE-004-note-isolation.md`
- 노트 전환 독립성과 충돌 처리

5. `FEATURE-005-editor-productivity.md`
- 편집 생산성(글자 수/전체 복사)

6. `FEATURE-006-note-list-reorder-and-group-move.md`
- 그룹 뷰 정렬 개선 및 리스트 기반 그룹 이동 계획

7. `FEATURE-007-auto-save-conflict-serialization.md`
- 자동 저장 직렬화 및 오탐 충돌 방지 계획

8. `FEATURE-008-note-public-sharing.md`
- 노트 공개 공유 기능

9. `FEATURE-009-admin-password-reset.md`
- 관리자 비밀번호 초기화 기능

10. `FEATURE-010-spell-check-integration.md`
- 맞춤법 검사 통합 기능

11. `FEATURE-011-note-trash-and-restore.md`
- 노트 휴지통 이동, 복원, 영구 삭제

## 공통 작성 규칙

1. 기능 문서는 아래 섹션을 포함한다.
- 목적/범위
- 사용자 시나리오
- 기능 요구사항
- API/데이터 계약
- 상태/예외 처리
- 테스트 기준(TDD 포인트)
- 오픈 이슈
- 연계 테스트 시나리오

2. PRD와 테스트 문서와의 추적성을 유지한다.
- PRD의 SR 번호 또는 후속 확장 필요 여부를 기능 문서에 명시한다.
- `docs/testing/TEST_PLAN.md`와 1:1 또는 1:N 매핑이 가능하도록 테스트 시나리오를 적는다.
- 현재 릴리스 범위 밖의 기능은 승격 시 필요한 테스트 추가 항목을 문서 안에 명시한다.
