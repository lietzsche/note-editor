...
메타 스킬은 개별 스킬 여러 개를 순서대로 묶는 상위 체인이다. “무엇을 하고 싶은가”가 명확하고, 매번 같은 흐름으로 끝까지 밀고 싶을 때 쓴다.

| Meta Skill | 언제 쓰는가 | 내부 기본 흐름 |
| --- | --- | --- |
| `$note-editor-meta-new-project-delivery` | 새로운 프로젝트 또는 큰 기능 축을 스펙/기획 의도부터 시작할 때 | 기획 -> 구현 -> 디자인 리뷰 -> 코드 리뷰 -> 문서화 |
| `$note-editor-meta-add-feature` | 기존 프로젝트에 새 기능을 추가할 때 | 기획 -> 구현 -> 디자인 리뷰 -> 코드 리뷰 -> 문서화 |
| `$note-editor-meta-change-feature` | 기존 기능의 동작이나 UX를 수정할 때 | 기획 -> 구현 -> 디자인 리뷰 -> 코드 리뷰 -> 문서화 |
| `$note-editor-meta-fix-bug` | 오류 수정 요청을 재현부터 문서화까지 처리할 때 | 오류 확인 -> 수정 -> 코드 리뷰 -> 문서화 |
| `$note-editor-meta-revise-design` | 화면/UI/UX 수정 요청을 방향 수립부터 반영까지 처리할 때 | 디자인 기획 -> 구현 -> 디자인 리뷰 -> 코드 리뷰 -> 문서화 |

문서화 단계는 별도 문서 전용 skill이 아니라 `$note-editor-apply-feedback`으로 코드, 테스트, 관련 `docs/` 문서를 함께 맞추는 방식으로 처리한다.
...
메타 스킬은 이 체인 패턴을 이름 붙여 재사용하는 방식이다. 반복되는 요청이면 긴 체인을 매번 직접 적기보다 메타 스킬 이름으로 요청하는 편이 안정적이다.
...
```text
$note-editor-meta-new-project-delivery를 사용해서 새 프로젝트의 스펙과 기획 의도를 주입할 테니 기획부터 구현, 리뷰, 문서화까지 진행해줘.
```

기능 추가:

```text
$note-editor-meta-add-feature를 사용해서 기존 프로젝트에 공유 링크 만료일 기능을 추가해줘.
```

기능 수정:

```text
$note-editor-meta-change-feature를 사용해서 기존 자동 저장 피드백 흐름을 수정해줘.
```

오류 수정:

```text
$note-editor-meta-fix-bug를 사용해서 노트 전환 중 저장 충돌 오류를 확인하고 수정한 뒤 문서화까지 해줘.
```

디자인 수정:

```text
$note-editor-meta-revise-design를 사용해서 모바일 노트 전환 UI를 다시 설계하고 반영해줘.
```
